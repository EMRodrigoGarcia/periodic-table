    ...
    $.fn.pte = function(p) {
        ...
        $.getJSON(params.data, function(data) {
            ...
            $.each(data.PERIODIC_TABLE.ATOM, function(key, val) {
                ...
                var ediv = $('<div>').attr('id', 'element_' + val.ATOMIC_NUMBER).addClass('pte_element').css('left',_left).css('top',_top).css('width',_width).css('height',_height).css('position','absolute').css('overflow','hidden').css('cursor','pointer').html('<div class="pte_number">' 
